...
{
public:
  ShapeFit(std::shared_ptr<rclcpp::Node> nh)
      : nh_(nh)
  {};
  ...
  */
  virtual void writeUrdf(std::shared_ptr<pugi::xml_node> lastLinkNode) = 0;
  ...
  /**
     ...
     @param lastLinkNode The XML node where collision-block shall be appended.
  */
  virtual void writeUrdfXacro(std::shared_ptr<pugi::xml_node> lastLinkNode) = 0;
};
...
class CapsuleFit: public ShapeFit
{
 public:
   ...
   */
  CapsuleFit(std::vector<double> points, Eigen::Vector3d translation,
  ...
  */
  void writeUrdf(std::shared_ptr<pugi::xml_node> lastLinkNode);
  ...
  /**
     ...
     Should be the last link in the XML.
  */
  void writeUrdfXacro(std::shared_ptr< pugi::xml_node > lastLinkNode);
...
class VersatileFitter
{
 public:
  ...
  /**
     ...
     @see Shape
  */
  void fit(std::string urdf_filename, std::string &output_filename, Shape shape,
...
class Urdf
{
 public:
   ...
   */
  void replaceMesh(std::shared_ptr<ShapeFit> shape);

  /*
   * Replace mesh based collision with capsule based xacro collision tags.
   */
  void replaceMeshWithXacro(std::shared_ptr<ShapeFit> shape);
...
public:
  ...
  MeshFile(std::shared_ptr<rclcpp::Node> nh) : nh_(nh){};
